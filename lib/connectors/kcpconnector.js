...
Connector.prototype.start = function(cb) {
    ...
    if (this.opts.useUDP) {
        ...
        self.on('disconnect', function(kcpsocket){
            var client = self.clientsForKcp[kcpsocket.opts.keyForClient];

            delete self.clientsForKcp[kcpsocket.opts.keyForClient];

            var index = self.clientsForKcpArray.indexOf(client);
            if (index > -1) {
                self.clientsForKcpArray.splice(index, 1);
            }

            var len = self.clientsForKcpArray.length;
            pomelocoder.getApp().set("kcpLen",len);
            logger.info("clientsForKcp len: %s", len);
        });
...
Connector.prototype.update = function() {
    for(var i=0;i<this.clientsForKcpArray.length;++i) {
        this.clientsForKcpArray[i].check();
    }
};
...
Connector.prototype.bindSocket = function(socket, address, port, msg) {
    ...
    if (msg) {
        ...
        conv = kcpHead.conv;
        if(conv == 123) {
            key = genKey(address, port);
            kcpsocket = self.clientsForKcp[key];
        } else {
